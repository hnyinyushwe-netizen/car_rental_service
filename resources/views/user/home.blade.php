...
<style>
  ...
  .driver-carousel .owl-nav button.owl-prev { left: -22px; }
  .driver-carousel .owl-nav button.owl-next { right: -22px; }
  ...
  @media (max-width: 768px) {
    ...
    .driver-carousel .owl-nav button.owl-prev { left: -12px; }
    .driver-carousel .owl-nav button.owl-next { right: -12px; }
  }
...
<section id="vehicles" class="bg-slate-100 py-12 fade-up">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-10">
      ...
      <button class="rounded-xl bg-slate-950 px-6 py-3 text-sm font-bold text-white transition-all duration-300 hover:bg-slate-800 hover:scale-105 hover:shadow-xl shrink-0">View Full Fleet &rarr;</button>
    </div>
    <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      @php
        $cars = [
          ['name' => 'Tesla Model 3', 'price' => 89, 'rating' => 4.9, 'img' => 'https://images.unsplash.com/photo-1606664515524-ed2f786a0bd6?auto=format&fit=crop&w=900&q=80', 'desc' => 'Electric sedan, autopilot ready'],
          ['name' => 'BMW X5 Executive', 'price' => 142, 'rating' => 4.8, 'img' => 'https://images.unsplash.com/photo-1617814076668-7772a3f5036a?auto=format&fit=crop&w=900&q=80', 'desc' => 'Luxury SUV, premium comfort'],
          ['name' => 'Mercedes C-Class', 'price' => 118, 'rating' => 4.7, 'img' => 'https://images.unsplash.com/photo-1618843479313-40f8afb4b4d8?auto=format&fit=crop&w=900&q=80', 'desc' => 'Business sedan, driver optional']
        ];
      @endphp
      @foreach($cars as $car)
      <article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-500 hover:shadow-xl hover:-translate-y-2 hover:border-cyan-300">
        <div class="relative overflow-hidden">
          <img class="h-56 w-full object-cover transition-transform duration-700 group-hover:scale-110" src="{{ $car['img'] }}" alt="{{ $car['name'] }}" />
          <span class="absolute right-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold text-slate-800 backdrop-blur-sm transition-all duration-300 group-hover:bg-cyan-400 group-hover:text-slate-950">⭐ {{ $car['rating'] }}</span>
        </div>
        <div class="p-3">
          <h3 class="text-xl font-black text-slate-900 transition-colors group-hover:text-cyan-600">{{ $car['name'] }}</h3>
          ...
          <div class="mt-5 flex items-center justify-between pt-4 border-t border-slate-100">
            <p class="font-black text-slate-950 text-lg">${{ $car['price'] }}<span class="text-sm font-semibold text-slate-500">/day</span></p>
            <button class="rounded-xl bg-cyan-400 px-5 py-2.5 text-sm font-bold text-slate-950 transition-all duration-300 hover:bg-cyan-300 hover:scale-105 shadow-sm shadow-cyan-400/10">View Details</button>
          </div>
        </div>
      </article>
      @endforeach
  ...
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-10">
      <div class="text-left max-w-3xl">
        ...
        <h2 class="mt-4 text-3xl font-black text-slate-950 sm:text-4xl tracking-tight">Meet Our Professional Chauffeurs</h2>
      ...
      <button class="rounded-xl bg-slate-950 px-6 py-3 text-sm font-bold text-white transition-all duration-300 hover:bg-slate-800 hover:scale-105 hover:shadow-xl shrink-0">View All Drivers &rarr;</button>
    </div>
    <div class="owl-carousel driver-carousel">
      @php
        $drivers = [
          ['name' => 'Michael Chen', 'role' => 'Luxury Sedan Specialist', 'exp' => '8 years', 'rating' => 4.9, 'img' => 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=400&q=80', 'languages' => 'English, Mandarin'],
          ['name' => 'Sophia Rodriguez', 'role' => 'Executive Chauffeur', 'exp' => '12 years', 'rating' => 5.0, 'img' => 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=400&q=80', 'languages' => 'English, Spanish'],
          ['name' => 'David Kim', 'role' => 'Airport Transfer Pro', 'exp' => '6 years', 'rating' => 4.8, 'img' => 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=400&q=80', 'languages' => 'English, Korean'],
          ['name' => 'Emma Wilson', 'role' => 'Corporate Driver', 'exp' => '10 years', 'rating' => 4.9, 'img' => 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80', 'languages' => 'English, French'],
          ['name' => 'James O\'Connor', 'role' => 'VIP Chauffeur', 'exp' => '15 years', 'rating' => 5.0, 'img' => 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=400&q=80', 'languages' => 'English, Irish'],
        ];
      @endphp
      @foreach($drivers as $driver)
      <div class="item px-2">
        <div class="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-md transition-all duration-300 hover:shadow-cyan-500/10 hover:-translate-y-2 hover:border-cyan-300">
          <img class="h-48 w-full object-cover transition-all duration-500" src="{{ $driver['img'] }}" alt="{{ $driver['name'] }}" />
          <div class="p-5">
            ...
            <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
              <span class="flex items-center gap-1"><i data-lucide="briefcase" class="h-3 w-3"></i> {{ $driver['exp'] }}</span>
            ...
            <div class="mt-5 flex justify-between items-center pt-3 border-t border-slate-100">
              <div class="flex gap-0.5 text-cyan-500">
                @for($i=1; $i<=5; $i++)
                  <i data-lucide="star" class="h-3.5 w-3.5 {{ $i <= $driver['rating'] ? 'fill-current' : 'opacity-30' }}"></i>
                @endfor
              </div>
              <button class="bg-cyan-400 hover:bg-cyan-300 text-slate-950 text-xs font-bold px-4 py-2 rounded-xl transition-all duration-300 hover:scale-105 shadow-sm shadow-cyan-400/10">Book Now</button>
            </div>
          </div>
        </div>
      </div>
...
<section id="sale" class="bg-slate-100 py-12 fade-up">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-10">
      <div class="text-left max-w-3xl">
        <p class="text-xs font-bold uppercase tracking-widest text-cyan-600 bg-cyan-50 inline-block px-3 py-1 rounded-full">Cars For Sale</p>
        <h2 class="mt-4 text-3xl font-black text-slate-950 sm:text-4xl tracking-tight">Direct From Verified Sellers</h2>
        <p class="mt-4 text-slate-600 text-base">Every individual unit profile maintains clear independent structural data records.</p>
      </div>
      <button class="rounded-xl bg-slate-950 px-6 py-3 text-sm font-bold text-white transition-all duration-300 hover:bg-slate-800 hover:scale-105 hover:shadow-xl shrink-0">View All Listings &rarr;</button>
    </div>
    <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      @php
        $saleVehicles = [
          ['title' => '2021 Honda Accord EX-L', 'miles' => '28,400', 'price' => '24,500', 'location' => 'Austin, TX', 'mpg' => '32 MPG', 'img' => 'https://images.unsplash.com/photo-1580273916550-e323be2ae537?auto=format&fit=crop&w=900&q=80'],
          ['title' => '2022 Audi A4 Premium', 'miles' => '19,100', 'price' => '32,800', 'location' => 'Miami, FL', 'mpg' => '31 MPG', 'img' => 'https://images.unsplash.com/photo-1606016159991-dfe4f2746ad5?auto=format&fit=crop&w=900&q=80'],
          ['title' => '2020 BMW 3 Series', 'miles' => '34,200', 'price' => '28,900', 'location' => 'Los Angeles, CA', 'mpg' => '30 MPG', 'img' => 'https://images.unsplash.com/photo-1556189250-72ba954cfc2b?auto=format&fit=crop&w=900&q=80']
        ];
      @endphp
      @foreach($saleVehicles as $saleVehicle)
      <article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md transition-all duration-500 hover:shadow-xl hover:-translate-y-2 hover:border-cyan-300">
        <div class="relative overflow-hidden">
          <img class="h-56 w-full object-cover transition-transform duration-700 group-hover:scale-110" src="{{ $saleVehicle['img'] }}" alt="{{ $saleVehicle['title'] }}" />
          <span class="absolute left-3 top-3 rounded-full bg-cyan-400 px-3 py-1 text-xs font-bold text-slate-900 shadow-md transition-all duration-300 group-hover:bg-cyan-500">For Sale</span>
        </div>
        <div class="p-5">
          <h3 class="text-xl font-black text-slate-900 transition-colors group-hover:text-cyan-600">{{ $saleVehicle['title'] }}</h3>
          <p class="mt-1 text-sm text-slate-500">1.5L Turbo &middot; {{ $saleVehicle['miles'] }} miles</p>
          <div class="mt-4 flex flex-wrap gap-3 border-t border-slate-100 pt-4 text-xs text-slate-600">
            <div class="flex items-center gap-1"><i data-lucide="fuel" class="h-3.5 w-3.5"></i> {{ $saleVehicle['mpg'] }}</div>
            ...
            <div class="flex items-center gap-1"><i data-lucide="map-pin" class="h-3.5 w-3.5"></i> {{ $saleVehicle['location'] }}</div>
          </div>
          <div class="mt-5 flex items-center justify-between pt-4 border-t border-slate-100">
            <p class="font-black text-slate-950 text-lg">${{ $saleVehicle['price'] }}</p>
            <button class="rounded-xl bg-cyan-400 px-5 py-2.5 text-sm font-bold text-slate-950 transition-all duration-300 hover:bg-cyan-300 hover:scale-105 shadow-sm shadow-cyan-400/10">View Listing</button>
          </div>
        </div>
      </article>
      @endforeach
  ...
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    <div class="rounded-2xl bg-gradient-to-br from-slate-800 to-slate-950 px-6 py-10 shadow-2xl transition-all duration-500 hover:shadow-cyan-900/30 sm:px-10 lg:flex lg:items-center lg:justify-between">
      ...
      <div class="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center lg:mt-0">
        <a href="mailto:support@skyline.demo" class="group inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-6 py-3.5 text-sm font-bold text-slate-950 transition-all duration-300 hover:bg-cyan-300 hover:scale-105 hover:shadow-lg pulse-hover">
